...
const categoryOrder=['ENTRADAS','LANCHES','SALGADOS ASSADOS','SALGADOS FRITOS','BOLO','DOCINHOS','BEBIDAS','E MAIS','ENTRADA DO(A) ANIVERSARIANTE'];
function renderCats(cats){ if(!cats) return ''; return categoryOrder.map(k=>{const items=cats[k]||[]; return items&&items.length?`<div class="cat"><h4>${k}</h4><ul>${items.map(i=>`<li>${i}</li>`).join('')}</ul></div>`:'';}).join(''); }
...
(function(){
  ...
  if(!toggle||!nav||!hamb) return;
  hamb.addEventListener('click',()=>setTimeout(()=>nav.classList.toggle('nav-open', toggle.checked),0));
  nav.querySelectorAll('a').forEach(a=>a.addEventListener('click',()=>{toggle.checked=false;nav.classList.remove('nav-open')}));
})();
