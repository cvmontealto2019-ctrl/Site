...
app.post('/agendadecoracao/salvar',agendaAuth,(req,res)=>{ 
  ...
  save(data); const [y,m]=obj.date.split('-'); res.redirect('/agendadecoracao/calendario?year='+Number(y)+'&month='+Number(m)+'&saved=1');
});
app.post('/agendadecoracao/excluir/:id',agendaAuth,(req,res)=>{ 
  ...
  if(ev){
    ...
    save(data);
    const [y,m]=ev.date.split('-'); return res.redirect('/agendadecoracao/calendario?year='+Number(y)+'&month='+Number(m)+'&saved=1');
  }
...
app.post('/agendadecoracao/restaurar/:id',agendaAuth,agendaAdmin,(req,res)=>{
  ...
  if(ev){
    ...
    save(data);
    const [y,m]=restored.date.split('-'); return res.redirect('/agendadecoracao/calendario?year='+Number(y)+'&month='+Number(m)+'&restored=1');
  }
