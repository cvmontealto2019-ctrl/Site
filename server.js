...
const instagramLink = link => {
  const l = String(link || '').trim();
  if (!l) return 'https://www.instagram.com/_buffetbirutapark/';
  if (l.startsWith('http://') || l.startsWith('https://')) return l;
  const user = l.replace('@','').replace(/^instagram\.com\//,'').replace(/\/$/,'');
...
app.use((req, res, next) => {
  ...
  res.locals.instagramLink = instagramLink;
...
app.post('/admin/testimonial', admin, (req, res) => {
  data.testimonials = data.testimonials || [];
  const obj = { name: req.body.name, text: req.body.text, active: !!req.body.active };
  if (req.body.id) { const t = data.testimonials.find(t => t.id == req.body.id); if (t) Object.assign(t, obj); }
  else data.testimonials.push({ id: data.next.testimonial++, ...obj });
  save(data); res.redirect('/admin/site#depoimentos-admin');
});
