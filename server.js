...
function load() {
  if (!fs.existsSync(DATA_FILE)) fs.writeFileSync(DATA_FILE, JSON.stringify({settings:{}, users:[], agenda_users:[], packages:[], banners:[], promotions:[], testimonials:[], leads:[], agenda_events:[], themes:[], next:{}}, null, 2));
...
function ensureData(d){
  ...
  d.agenda_users ||= [{id:1, login:'decoracao', password:'Biruta@2026', name:'Admin Decoração', role:'admin'}];
  d.packages ||= []; d.banners ||= []; d.promotions ||= []; d.testimonials ||= []; d.leads ||= [];
  d.agenda_events ||= []; d.themes ||= DEFAULT_THEMES;
  d.next ||= {};
  d.next.package ||= 1; d.next.banner ||= 1; d.next.promotion ||= 1; d.next.testimonial ||= 1; d.next.lead ||= 1; d.next.agenda ||= 1; d.next.user ||= (Math.max(0,...d.users.map(u=>u.id||0))+1); d.next.agenda_user ||= (Math.max(0,...d.agenda_users.map(u=>u.id||0))+1);
  return d;
}
...
app.use((req,res,next)=>{ data = ensureData(load()); res.locals.s=data.settings||{}; res.locals.current_user=req.session.user||null; res.locals.agenda_user=req.session.agendaUser||null; res.locals.promoLink=promoLink; res.locals.instagramLink=instagramLink; res.locals.categoriesOrder=categoriesOrder; res.locals.whatsUrl=whatsUrl; res.locals.canEditAgenda=()=>canEditAgenda(req); next(); });
...
app.get('/agendadecoracao',agendaAuth,(req,res)=>{ 
  ...
  res.render('agenda',{year,month,events:data.agenda_events||[],themes:data.themes||[],saved:req.query.saved,stats:agendaStats()}); 
});
...
app.post('/agendadecoracao/excluir/:id',agendaAuth,(req,res)=>{ 
  if(!canEditAgenda(req)) return res.status(403).send('Seu usuário pode apenas visualizar a agenda.');
  const ev=data.agenda_events.find(e=>e.id==req.params.id); data.agenda_events=data.agenda_events.filter(e=>e.id!=req.params.id); save(data); 
  if(ev){const [y,m]=ev.date.split('-'); return res.redirect('/agendadecoracao?year='+Number(y)+'&month='+Number(m)+'&saved=1');} res.redirect('/agendadecoracao');
});
